...
$$
\begin{aligned}
\widehat{\text{Fishing Mortality}} &= \gamma_0 + \gamma_1 \text{Size}_\text{centered} + \gamma_2 \text{Below cutoff} + \omega \\
\text{Exit exam} &= \beta_0 + \beta_1 \text{Size}_\text{centered} + \beta_2 \widehat{\text{Fished}} + \epsilon
\end{aligned}
$$
...
```{r rdrobust-fuzzy}
rdrobust(y = crab_data$fishing_mortality, x = tutoring$entrance_exam,
